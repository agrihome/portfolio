    ...
    <section id="services" className="py-48 flex flex-col items-center w-full" aria-label="Services">
      ...
      <div className="services-list w-full flex flex-col items-center">
        {SERVICES.map((s) => (
          <div 
            key={s.num} 
            className="service-item w-full flex items-center justify-center py-10 md:py-16 cursor-pointer group transition-colors duration-300 hover:bg-[var(--ink)]/5"
          >
            <div className="flex items-center gap-6 md:gap-16">
              <span className="font-display text-5xl md:text-8xl font-bold uppercase tracking-tighter text-[var(--ink)] transition-colors duration-300 group-hover:text-pink-500">
                {s.name}
              </span>
            </div>
          </div>
