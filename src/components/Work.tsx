...
const SlideShowCard = ({ p, i, slideSet }: { p: any, i: number, slideSet?: string[] }) => {
  const [currentSlide, setCurrentSlide] = useState(0)

  useEffect(() => {
    if (!slideSet) return

    // Random interval between 3000 and 5000ms
    const randomInterval = Math.floor(Math.random() * 2000) + 3000
  ...
  return (
    ...
    >
      <div className="aspect-square w-full  overflow-hidden relative border border-(--ink)/20">
        {slideSet ? (
          <AnimatePresence mode="popLayout">
            <motion.img
              key={currentSlide}
              src={slideSet[currentSlide]}
  ...
  return (
    <section id="work" className="py-24 flex flex-col items-center w-full" aria-label="Selected work">
      <div className="w-full">
        ...
        >
          <Text3DFlip className="headline-lg tracking-tighter inline-block" secondaryChildren="MY WORK">
            MY WORK   </Text3DFlip>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8">
          {PROJECTS.map((p, i) => (
            <SlideShowCard key={p.id} p={p} i={i} slideSet={slideSets[i]} />
          ))}
        </div>
      </div>
    </section>
